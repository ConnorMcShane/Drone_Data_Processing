...
def track(detections_file, video_file, fps):
    ...
    for i in range(number_of_frames):
        ...
        if centroids.size != 0 and new_centroids.size != 0:
            ...
            for v in tracked_vehicles:
                ...
                if i-(lost_time) < v.lastFrame < i and v.visible_count > legit_time and img_w*0.01 < v.x < img_w*0.99:                   
    ...
    tracked_unfiltered_file.append(['timestep(s)', 'vehicle_id', 'x_centre_pos(m)', 'y_centre_pos(m)', 'length(m)', 'width(m)'])
    ...
    for i in range(number_of_frames):
        for v_index in range(len(tracked_vehicles)):
            if tracked_vehicles[v_index].startFrame < i < tracked_vehicles[v_index].lastFrame:
                ...
                vid = v_index
                vw = round((w_avg[v_index]),3)
                vh = round((h_avg[v_index]),3)
                vx = round((((x[v_index][f_index]))+(vw/2)),3)
                vy = round((((y[v_index][f_index]))+(vh/2)),3)
                tracked_unfiltered_file.append([str(round(i/fps,3)), str(vid), str(vx), str(vy), str(vw), str(vh)])

    tracked_unfiltered_filePath = r'C:/Users/mlpre/Desktop/DJI_0025_TRACKED.csv'
    with open(tracked_unfiltered_filePath, 'w', newline='') as myfile:
        wr = csv.writer(myfile)
        wr.writerows(tracked_unfiltered_file)

track(r'C:/Users/mlpre/Desktop/DJI_0025_DETECTIONS.csv', r'C:/Users/mlpre/Desktop/DJI_0025_DOWNSAMPLE.avi', 29)