...
from Object_Detection.Deep_Learning.YoloV3.darknet.darknet import performDetect
import random
import cv2
import time
import numpy as np
...
class FileVideoStream:
    ...
    def stop(self):
	    self.stopped = True
def detect(darknet_root = '', detections_folder = '', video_path = '', fps = 30, scale = 1, make_video = False, show_images = True):
    # GET FILE PATHS
    root = Tk()
    root.withdraw()
    if darknet_root == '':
    ...
    autoplex_data = 'classes= 1 \ntrain  = ' + darknet_root + '/data/autoplex/train.txt\nvalid  = ' + darknet_root + '/data/autoplex/test.txt\nnames = ' + darknet_root + '/data/autoplex/autoplex.names\nbackup = ' + darknet_root + '/backup/autoplex/'
    with open(darknet_root + "/data/autoplex/autoplex.data", 'w') as file:
        file.write(autoplex_data)
